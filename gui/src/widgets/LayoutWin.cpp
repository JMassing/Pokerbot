...
namespace gui
{
    ...
    void LayoutWin::content()
    {
        // Give a Constant label width, the rest goes to widget size, e.g. width of slider bar
        ImGui::PushItemWidth(ImGui::GetFontSize() * -14);  

        ImGui::Checkbox("Show Card Outline", &this->config_.show_cards);ImGui::SameLine(); this->helpMarker("If checked, outline of cards and cards rank/suit are displayed in the live image.");  
        ...
        ImGui::Checkbox("Show Camera Controls", &this->config_.show_camera_control);
        ImGui::Checkbox("Show Image Processing Settings", &this->config_.show_im_proc_settings_window);
        ImGui::Checkbox("Show Card Images", &this->config_.show_card_images); ImGui::SameLine(); this->helpMarker("If checked, card images extracted from live frame are displayed.");
        ImGui::Checkbox("Show Rank Images", &this->config_.show_rank_images); ImGui::SameLine(); this->helpMarker("If checked, binarized rank images extracted from card images are displayed.");
        ImGui::Checkbox("Show Suit Images", &this->config_.show_suit_images); ImGui::SameLine(); this->helpMarker("If checked, binarized suit images extracted from card images are displayed.");
        ...
        ImGui::SliderInt("Card Image Size", &this->config_.card_image_height_percent, 25, 175); ImGui::SameLine(); this->helpMarker("Size of displayed card images in %. CTRL+click to input value.");
        this->enforceBoundaries(25, 175, this->config_.card_image_height_percent);
        ImGui::SliderInt("Rank/Suit Image Size", &this->config_.card_rank_suit_height_percent, 25, 175);ImGui::SameLine(); this->helpMarker("Size of displayed rank and suit images in %. CTRL+click to input value.");
        ...
        {
            this->addWindow("##saveSettingsLayout",this->show_ask_for_save_,[this, &save](){this->askForSave(save, this->show_ask_for_save_layout_, "layout");});
        }
        if(save == true)
        {
